...
def get_mse(parameters, alpha, T):
    omega1 = parameters[0]
    omega2 = parameters[1]
    omega3 = parameters[2]
    omega4 = parameters[3]
...
def get_mse_multi_data(parameters, X, T, Y):
    omega1 = parameters[0]
    omega2 = parameters[1]
    omega3 = parameters[2]
    omega4 = parameters[3]
    ...
    for _ in range(n_datasets):
        mse_values.append(get_mse(parameters, alpha, T))
    return float(np.mean(mse_values))


def get_mse_over_set_training_datas(parameters):
    omse_values = []
    for i in range(z):
        omse_values.append(get_mse_multi_data(parameters, X[i*N : (i+1)*N], T[i*M : (i+1)* M], Y[i*N : (i+1)*N]))
    return float(np.mean(omse_values))
...
start_parameters = np.array([10.0, 10, 5, 10,0.05])
bounds = ((1, 50), (0.1, 10), (0.1, 5), (0.1, 10), (1e-6, 0.1))
result = scopt.minimize(get_mse_over_set_training_datas, start_parameters, method = 'L-BFGS-B', bounds = bounds)
