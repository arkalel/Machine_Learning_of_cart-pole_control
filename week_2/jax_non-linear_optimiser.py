...
def get_mse(parameters):
    ...
    for i in range(50):
        cp = random.uniform(-2.5, 2.5)
        cv = random.uniform(-10, 10)
        pa = random.uniform(-np.pi, np.pi)
        pv = random.uniform(-15, 15)
...
start_parameters = np.array([40, 3, 0.4 , 3 , 0.0001])
bounds = ((1, 50), (1, 50), (0.1, 10), (1, 50), (1e-6, 0.1))
eps =(0.05 * start_parameters)
result = scopt.minimize(get_averaged_mse, start_parameters, method = 'L-BFGS-B', bounds = bounds, options={"maxiter": 200, "eps": eps, "ftol": 1e-4})
